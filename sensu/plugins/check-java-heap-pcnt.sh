...
# Also make sure the user "sensu" can without password
...
PID=$(sudo /usr/java/default/bin/jps | grep $NAME | awk '{ print $1}')

#Get heap capacity of JVM
TotalHeap=$(sudo /usr/java/default/bin/jstat -gccapacity $PID  | tail -n 1 | awk '{ print ($4 + $5 + $6 + $10) / 1024 }')

#Determine amount of used heap JVM is using
UsedHeap=$(sudo /usr/java/default/bin/jstat -gc $PID  | tail -n 1 | awk '{ print ($3 + $4 + $6 + $8 + $10) / 1024 }')
